...
int cache_remove_url(cache_request_rec *cache, request_rec *r)
{
    ...
    if (count != 0) {
	elapsed /= count;
	ap_log_error(APLOG_MARK, APLOG_CRIT, 0, r -> server, "%lf REMOVE URL", elapsed);	
    }
    return OK;
}
...
int cache_select(cache_request_rec *cache, request_rec *r)
{
	...
	clock_gettime(CLOCK_MONOTONIC, &start);
		...
		count++;
    		...
    		if (count != 0) {
			elapsed /= count;
			ap_log_error(APLOG_MARK, APLOG_CRIT, 0, r -> server, "%lf OPEN_ENTITY", elapsed);	
   		 }
...
int cache_invalidate(cache_request_rec *cache, request_rec *r)
{
    ...
    if (count != 0) {
	elapsed /= count;
	ap_log_error(APLOG_MARK, APLOG_CRIT, 0, r -> server, "%lf OPEN_ENTITY_2", elapsed);	
    }
