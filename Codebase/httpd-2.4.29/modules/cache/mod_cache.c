        ...
        cache_server_conf *conf, cache_request_rec *cache)
{
	...
	elapsed = (finish.tv_nsec - start.tv_nsec) / 500000000.0;
	ap_log_rerror(APLOG_MARK, APLOG_CRIT, rv, f -> r, "%lf COMMIT_ENTITY", elapsed);	
                break;
            }
        }
...
static apr_status_t cache_save_filter(ap_filter_t *f, apr_bucket_brigade *in)
{
	...
	clock_gettime(CLOCK_MONOTONIC, &finish);
	elapsed = (finish.tv_nsec - start.tv_nsec) / 500000000.0;
	ap_log_error(APLOG_MARK, APLOG_CRIT, 0, r -> server, "%lf STORE_HEADER", elapsed);	
	...
	clock_gettime(CLOCK_MONOTONIC, &finish);
	elapsed = (finish.tv_nsec - start.tv_nsec) / 500000000.0;
	ap_log_error(APLOG_MARK, APLOG_CRIT, 0, r -> server, "%lf COMMIT_ENTITY", elapsed);	
        }
	...
	clock_gettime(CLOCK_MONOTONIC, &finish);
	elapsed += (finish.tv_nsec - start.tv_nsec) / 500000000.0;
	ap_log_error(APLOG_MARK, APLOG_CRIT, 0, r -> server, "%lf REMOVE_URL INSIDE", elapsed);	
            if (rv != OK) {
